<?php defined('K2F') or die;
	...
	abstract class WKCORE {
		...
		protected static function exe($bin){
			if(!self::$exe){
				...
				if(!file_exists(self::$exe)){
					throw new Exception('WKHTMLTOX error: Could not find executable for the current platform.');
					return null;
				}
		...
		public function render(){
			...
			if($this->result['stdout']=='' && $this->result['return']==0)
				throw new Exception('WKHTMLTOX error: program returned empty result.');
			// handle execution return codes (only 0 means success).
			switch($this->result['return']){
				case -1073741515:
					throw new Exception('WKHTMLTOX runtime error: some required runtime dlls are missing.');
					return false;
				case 7:
					throw new Exception('WKHTMLTOX system error 7: malformed executable (execution failure).');
					return false;
				case 3:
					throw new Exception('WKHTMLTOX error 401: recieved "unauthorized access" for target url.');
					return false;
				case 2:
					throw new Exception('WKHTMLTOX error 404: recieved "file not found" for target url.');
					return false;
				case 1:
					throw new Exception('WKHTMLTOX system error 1: a generic error occured, possibly due to wrong parameters.');
					return false;
				case 0:
					return true;
				default:
					throw new Exception('WKHTMLTOX error '.$this->result['return'].': Unidentified error code.');
					return false;
			}
		}
		/**
			...
			switch($mode){
				case self::OUT_DOWNLOAD:
					if(!@headers_sent()){
						...
						throw new Exception('WKHTMLTOX error: HTTP headers were already sent.');
						return false;
					}
				...
				default:
					throw new Exception('WKHTMLTOX error: unsupported or unrecognized $mode value passed to output().');
					return false;
			}
		}
	}
